...
@livewireScripts()
    ...
    @if(session()->has('error'))
        <x-noti type="error" message="{{ session()->get('error') }}"></x-noti>
    @endif

    @if(session()->has('success'))
        <x-noti type="success" message="{{ session()->get('success') }}"></x-noti>
    @endif
                ...
                <div class="my-4">
                    ...
                    <select wire:model="product_category.parent_id" id="parent_id" name="parent_id" class="block w-full rounded">
                        ...
                        @if (!empty($product_categories))
                            @include('livewire.tenant.backend.product.recursive-select', ['product_categories' => $product_categories, 'prefix'       => ''])
                        @endif 
